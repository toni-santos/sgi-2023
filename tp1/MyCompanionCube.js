...
class MyCompanionCube extends THREE.Object3D {
    ...
    constructor(app, edge = 5, heartColor = 0xff88ff) {
        ...
        this.accentMaterial = new THREE.MeshPhongMaterial({
            shininess: 100,
        ...
        this.extrudeSettings = {
            ...
            steps: 2,
            bevelSize: 0.3,
            bevelThickness: (0.1 * this.edge) / 4
        };
        this.addHeartSlots();
        this.addCornerArmor();
        this.addEdgeArmor();
    ...
    addHeartSlots() {
        const circleSlot = new THREE.CylinderGeometry(
            this.edge / 4,
            this.edge / 4,
            this.edge * 1.05
        );
        const slotMesh = new THREE.Mesh(circleSlot, this.accentMaterial);
    ...
    addLines(slot) {
        const lineHole = new THREE.CylinderGeometry(
            this.edge / 70,
            this.edge / 70,
            this.edge,
            4
        );
        let lineMesh = new THREE.Mesh(lineHole, this.heartMaterial);
    ...
    addLinePair(line, slot, rotationVector) {
        line.rotation.set(
            ...arrayMult(new Array(3).fill(Math.PI / 2), rotationVector)
        );
    ...
    positionBar(delta) {
        ...
        armorMesh.position.set(
            ...arrayMult(delta, Array(3).fill(this.edge / 2))
        );
    ...
    drawHeart(x = 0, y = 0) {
        ...
        const heart = new THREE.ExtrudeGeometry(
            heartLine,
            this.extrudeSettings
        );
