...
import { MyReader } from "./parser/MyReader.js";
    ...
    */
        ...
        for (const textureKey in textures) {
            ...
            if (!textureData.mipmaps && textureData.mipmap0) {
                ...
                if (textureData.mipmap0)
                    this.loadMipmap(texture, textureData.mipmap0, 0);
                if (textureData.mipmap1)
                    this.loadMipmap(texture, textureData.mipmap1, 1);
                if (textureData.mipmap2)
                    this.loadMipmap(texture, textureData.mipmap2, 2);
                if (textureData.mipmap3)
                    this.loadMipmap(texture, textureData.mipmap3, 3);
                if (textureData.mipmap4)
                    this.loadMipmap(texture, textureData.mipmap4, 4);
                if (textureData.mipmap5)
                    this.loadMipmap(texture, textureData.mipmap5, 5);
                if (textureData.mipmap6)
                    this.loadMipmap(texture, textureData.mipmap6, 6);
                if (textureData.mipmap7)
                    this.loadMipmap(texture, textureData.mipmap7, 7);
            } else {
        ...
        for (const materialKey in materials) {
            ...
            if (texture !== undefined) {
                ...
                texture.repeat = new THREE.Vector2(
                    1 / materialData.texlength_s,
                    1 / materialData.texlength_t
                );
            }
            const material = !materialData.bumpref
                ? new THREE.MeshPhongMaterial({
                      color: materialData.color,
                      specular: materialData.specular,
                      emissive: materialData.emissive,
                      shininess: materialData.shininess,
                      side: materialData.twosided
                          ? THREE.DoubleSide
                          : THREE.FrontSide,
                ...
                : new THREE.MeshPhongMaterial({
                      ...
                      specularMap:
                          this.textures[materialData.specularref] ?? null
                  });
    ...
    convertObject(nodeId, objects, visited, parentData = undefined, idx = 0) {
        ...
        if (nodeData?.type !== "lod") {
            ...
            if (nodeData === undefined) {
                if (parentData.children[index].type?.includes("light"))
                    return this.createLight(parentData.children[index]);
                return this.createPrimitive(nodeId, parentData, index);
            }

            if (nodeData.transformations.length > 0) {
                const newMatrix = this.transformObject(
            ...
            for (const childId of this.sceneGraph.getNode(nodeId)) {
                ...
                if (
                    nodeId !== this.data.rootId &&
                    parentData.materialIds?.length !== 0 &&
                    nodeData.materialIds?.length === 0
                ) {
                    nodeData.materialIds = parentData.materialIds;
                }
                const childMesh = this.convertObject(
                    childId,
                    objects,
                    visited,
                    nodeData,
        ...
        for (const transformation of transformations) {
            switch (transformation.type) {
                case "T":
                    ...
                    break;
                case "R":
                    const rotation = transformation.rotation.map(
                        (x) => (x * Math.PI) / 180
                    );
    ...
    createPrimitive(nodeId, objectData, index) {
        ...
        switch (nodeId) {
            ...
            case "sphere":
                ...
                geometry = new THREE.SphereGeometry(
                    radius,
                    representation.slices ?? 32,
                    representation.stacks ?? 16,
                    phistart,
                    philength,
                    thetastart,
                    thetalength
            ...
            case "box":
                ...
                center_x =
                    (representation.xyz2[0] + representation.xyz1[0]) / 2;
                center_y =
                    (representation.xyz2[1] + representation.xyz1[1]) / 2;
                center_z =
                    (representation.xyz2[2] + representation.xyz1[2]) / 2;
            ...
            case "nurbs":
                center_x =
                    (representation.controlpoints[
                        representation.controlpoints.length - 1
                    ].xx +
                        representation.controlpoints[0].xx) /
                    2;
                center_y =
                    (representation.controlpoints[
                        representation.controlpoints.length - 1
                    ].yy +
                        representation.controlpoints[0].yy) /
                    2;
                center_z =
                    (representation.controlpoints[
                        representation.controlpoints.length - 1
                    ].zz +
                        representation.controlpoints[0].zz) /
                    2;
                ...
                for (let stack = 0; stack <= representation.stacks; stack++) {
                    for (
                        ...
                        const points = [
                            ...
                            [
                                Math.cos(angle * (slice + 1)) *
                                    (step * (stack + 1)),
                                Math.sin(angle * (slice + 1)) *
                                    (step * (stack + 1)),
                                0
                            ],
                            [
                        ...
                        const colors_v = [
                            new THREE.Color()
                                .lerpColors(start_c, end_c, stack * step)
                                .toArray(),
                            new THREE.Color()
                                .lerpColors(start_c, end_c, (stack + 1) * step)
                                .toArray()
                        ];
            ...
            case "circuit":
                ...
                for (const obj of this.reader.createObstacles(representation.obstacles)) {
                    group.add(obj);
                }
        ...
        const mesh = new THREE.Mesh(
            geometry,
            this.materials[objectData.materialIds[0]]
        );
        mesh.castShadow = true;
        mesh.receiveShadow = true;
        return mesh;
    ...
    createSceneGraph(objects) {
        const rootNodeChildIds = objects[this.data.rootId].children.map(
            (node) => node.id ?? node.subtype
        );
        const graph = new MyGraph(
            Object.keys(objects)
                .concat(this.data.primitiveIds)
                .concat(rootNodeChildIds)
                .concat(["pointlight", "directionallight", "spotlight"])
        );
        for (const objectKey in objects) {
            const objectData = objects[objectKey];
            for (const child of objectData.children) {
                const id = child.type?.includes("light")
                    ? child.type
                    : child.id ?? child.subtype;
    ...
    changeColor(color, mesh, lightOnly) {
        this.meshes[mesh].map((obj) =>
            obj.children.map((child) => {
                lightOnly
                    ? child.type.toLowerCase().includes("light")
                        ? (child.color = color)
                        : false
                    : (child.color = color);
            })
    ...
    defineControls() {
        ...
        this.controlsTargets = {
            ...
            "Vinyl Player":
                this.getWorldPos(this.meshes["vinylPlayer"][0]) ??
                new THREE.Vector3(0, 0, 0),
            "Left Table":
                this.getWorldPos(this.meshes["table"][0]) ??
                new THREE.Vector3(0, 0, 0),
            Poster:
                this.getWorldPos(this.meshes["poster"][0]) ??
                new THREE.Vector3(0, 0, 0),
            TV:
                this.getWorldPos(this.meshes["tv"][0]) ??
                new THREE.Vector3(0, 0, 0)
        };
